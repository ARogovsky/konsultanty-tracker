import type React from 'react';

// Team member card component
const TeamMember = ({
  name,
  role,
  image,
  description,
}: {
  name: string;
  role: string;
  image: string;
  description: string;
}) => {
  return (
    <div className="overflow-hidden rounded-xl bg-white shadow-lg transition-transform hover:scale-105">
      <img src={image} alt={name} className="h-120 w-full object-cover" />
      <div className="p-6">
        <h3 className="text-xl font-bold text-mipt-purple-dark">{name}</h3>
        <p className="mb-3 text-mipt-pink">{role}</p>
        <p className="text-gray-600">{description}</p>
      </div>
    </div>
  );
};

const TeamSection: React.FC = () => {
  const teamMembers = [
    {
      name: 'Александр Иванов',
      role: 'Руководитель программы',
      image: '/assets/images/pic_001.png',
      description:
        'Опытный трекер с более чем 10-летним стажем работы с технологическими стартапами и корпорациями.',
    },
    {
      name: 'Екатерина Смирнова',
      role: 'Ведущий трекер',
      image: '/assets/images/pic_002.png',
      description:
        'Эксперт по работе с инновационными проектами и внутренними стартапами в крупных компаниях.',
    },
    {
      name: 'Михаил Петров',
      role: 'Трекер-методолог',
      image: '/assets/images/pic_003.png',
      description:
        'Специалист по разработке методологий трекинга и автор учебных материалов курса.',
    },
  ];

  return (
    <section id="team" className="bg-mipt-light py-16 md:py-24">
      <div className="container mx-auto px-4">
        <h2 className="mb-2 text-center text-3xl font-bold text-mipt-purple-dark md:text-4xl">
          Команда программы
        </h2>
        <p className="mb-12 text-center text-lg text-gray-600">
          Наши эксперты имеют обширный опыт работы в сфере инноваций и трекинга
        </p>

        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {teamMembers.map((member) => (
            <TeamMember
              key={member.name}
              name={member.name}
              role={member.role}
              image={member.image}
              description={member.description}
            />
          ))}
        </div>

        <div className="mt-16 text-center">
          <h3 className="mb-4 text-2xl font-bold text-mipt-purple-dark">
            Присоединяйтесь к нашей команде профессионалов
          </h3>
          <p className="mb-8 text-gray-600">
            Программа обучения разработана опытными трекерами, которые делятся своими практическими
            знаниями и подходами
          </p>
          <button className="rounded-lg bg-mipt-purple px-8 py-3 font-medium text-white hover:bg-mipt-purple-dark">
            Узнать больше о преподавателях
          </button>
        </div>
      </div>
    </section>
  );
};

export default TeamSection;
